...
class TyphoonDataset(Dataset):
    def __init__(self, csv_file, root_dir_field, root_dir_value, root_dir_track, root_dir_target, track_model=None,
                 augment=False, field_normalizer=None,
    ...
    def augment_data(self, value_input):
        if self.augment:
            perturbation = random.uniform(-0.1, 0.1)  # 简单的随机扰动
            value_input += perturbation
        return value_input
    ...
    def __getitem__(self, idx):
        ...
        value_input = self.augment_data(value_input)
...
def get_dataloader(csv_file, batch_size, root_dir_field, root_dir_value, root_dir_track, root_dir_target, shuffle=True,
                   track_model=None, augment=False, field_normalizer=None, intensity_normalizer=None,
    ...
    dataset = TyphoonDataset(csv_file=csv_file, root_dir_field=root_dir_field, root_dir_value=root_dir_value,
                             ...
                             field_normalizer=field_normalizer, augment=augment, track_model=track_model,
...
#                               root_dir_track, root_dir_target, shuffle=True, augment=True,
