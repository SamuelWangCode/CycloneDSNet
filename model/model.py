...
class SimpleSAW(nn.Module):
    def __init__(self, height, width, N, center_weight):
        super(SimpleSAW, self).__init__()
...
class BCModel(nn.Module):
    ...
    def __init__(self, in_shape1, in_shape2, hid_S=16, hid_T=256, N_S=4, N_T=4, output_dim=3,
        ...
        self.gap = GAP()
        self.value_encoder = TimeDistributedValueEncoder(C2, C2) 
...
class CNNEncoder(nn.Module):
    def __init__(self, C_in, C_hid, N_S, spatio_kernel=3, act_inplace=True):
        super(CNNEncoder, self).__init__()
        layers = []
        for i in range(N_S):
            layers.append(
                nn.Conv2d(
                    in_channels=C_in if i == 0 else C_hid,
                    out_channels=C_hid,
                    kernel_size=spatio_kernel,
                    padding=spatio_kernel // 2
                )
            )
            layers.append(nn.ReLU(inplace=act_inplace))
...
class DownscaleDecoder(nn.Module):
    def __init__(self, in_channels, cond_dim, out_channels,
                 num_upsample=2, scale_factor=2,
                 target_size=None):
        super(DownscaleDecoder, self).__init__()

        self.cond_proj = nn.Linear(cond_dim, in_channels)

        blocks = []
        for i in range(num_upsample):
            if (i == num_upsample - 1) and (target_size is not None):
            ...
            blocks.append(
                nn.Sequential(
                    upsample,
                    nn.Conv2d(in_channels, in_channels, kernel_size=3, padding=1, padding_mode='zeros'),
                    nn.ReLU(inplace=True),
                )
            )
        self.blocks = nn.ModuleList(blocks)

        self.out_conv = nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1, padding_mode='zeros')

    def forward(self, feat, cond):
        """
        [修改 3] T-to-1 模式
        feat: (B, C_in, H, W)  <-- 单帧特征
        cond: (B, C_cond)      <-- 单帧条件
        """
        B, C, H, W = feat.shape

        cond_emb = self.cond_proj(cond)  # (B, C_in)
        cond_emb = cond_emb.view(B, C, 1, 1)  # (B, C_in, 1, 1)
        ...
        out = self.out_conv(feat)  # (B, C_out, H_hr, W_hr)

        return out.unsqueeze(1)


class BCDownscaleModel(nn.Module):
    def __init__(self, in_shape1, in_shape2,
                 hid_S=16, hid_T=256, N_S=4, N_T=4,
                 out_channels=8,
                 model_type='incepu', mlp_ratio=8., drop=0.0, drop_path=0.0,
                 spatio_kernel_enc=3, act_inplace=True,
                 center_weight=1.0, N=10,
                 num_upsample=2, scale_factor=2,
                 hr_size=None,
                 **kwargs):
        super(BCDownscaleModel, self).__init__()
        ...
        self.spatial_attention_weight = SpatialAttentionWeight(
            height=H, width=W, N=N, center_weight=center_weight
        )

        self.mid_layer = MidIncepNet(T * hid_S, hid_T, N_T) if model_type == 'incepu' else \
            MidMetaNet(T * hid_S, hid_T, N_T,
                       input_resolution=(H, W), model_type=model_type,
                       mlp_ratio=mlp_ratio, drop=drop, drop_path=drop_path)

        self.value_encoder = TimeDistributedValueEncoder(C2, C2)

        self.hr_decoder = DownscaleDecoder(
            in_channels=hid_S,
            cond_dim=C2,
    ...
    def forward(self, x1, x2, **kwargs):
        ...
        embed1, _ = self.encoder1(x1.view(B * T, C1, H, W))

        feat = self.spatial_attention_weight(embed1)
        _, C_hid, H_s, W_s = feat.shape
        feat = feat.view(B, T, C_hid, H_s, W_s)

        feat = self.mid_layer(feat)  # (B, T, C, H, W)

        feat_last = feat[:, -1, :, :, :]  # (B, C_hid, H, W)

        cond_seq = self.value_encoder(x2)  # (B, T, C2)
        cond_last = cond_seq[:, -1, :]  # (B, C2)

        out_hr = self.hr_decoder(feat_last, cond_last)  # (B, 1, C, H, W)
