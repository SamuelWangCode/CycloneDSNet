...
def load_input_from_nc(input_path):
    ...
    pl = pl.transpose('level', 'latitude', 'longitude')
    sfc = sfc.transpose('latitude', 'longitude')
...
def predict(model_wrapper, input_path):
    ...
    with torch.no_grad():
        for i in range(int(lead_time / hour_step)):
            output = forecast_model(outputs[i])

            outputs.append(output)
    # 统一处理归一化还原
    final_outputs = []
    for t in outputs:
        final_outputs.append(model_wrapper.normalise(t, reverse=True))

    return final_outputs


if __name__ == '__main__':
    root_path = "/bigdata3/WangGuanSong/Weaformer/all_models/weaformer_v2.0/"
    df = pd.read_csv('./data_file/forecast_24h_patch.csv')
    df['Start Date'] = pd.to_datetime(df['Start Date'], format='%Y%m%d%H')

    forecast_tasks = df[['Start Date']].drop_duplicates()
    ...
    for _, row in forecast_tasks.iterrows():
        ...
        path_1 = f"/large/ERA5/model_inputs/{start_date.strftime('%Y')}/{start_date.strftime('%m')}/{start_date.strftime('%Y%m%d%H')}/"
        path_2 = f"/bigdata1/TianXing_input/{start_date.strftime('%Y')}/{start_date.strftime('%m')}/{start_date.strftime('%Y%m%d%H')}/"

        if os.path.exists(path_1):
            input_path = path_1
        else:
            input_path = path_2
        if not os.path.exists(f'/bigdata1/TianXing_forecast/prediction_{start_date.strftime("%Y%m%dT%H")}.nc'):
            ...
            output_filename = f'/bigdata1/TianXing_forecast/prediction_{start_date.strftime("%Y%m%dT%H")}.nc'
            dataset.to_netcdf(output_filename)
            print(f"Saved prediction to {output_filename}")
            count = count + 1
        else:
            count = count + 1
            print(f'/bigdata1/TianXing_forecast/prediction_{start_date.strftime("%Y%m%dT%H")}.nc exists.')
